...
class MVTypeConverter extends TypeConverter{
	...
	public function __construct(BlockItemIdMap $blockItemIdMap, MVBlockTranslator $blockTranslator, ItemTypeDictionary $itemTypeDictionary, MVItemTranslator $ItemTranslator, SkinAdapter $skinAdapter, ?int $protocolId = null){
		$protocolId ??= ProtocolInfo::CURRENT_PROTOCOL;

		if(method_exists(TypeConverter::class, "__construct")){
			$parentCtor = new \ReflectionMethod(TypeConverter::class, "__construct");
			if($parentCtor->getNumberOfParameters() >= 1){
	...
	public function coreGameModeToProtocol(GameMode $gamemode) : int{
		if(method_exists(GameMode::class, "SURVIVAL") && method_exists($gamemode, "id")){
	...
	public function protocolGameModeToCore(int $gameMode) : ?GameMode{
		if(method_exists(GameMode::class, "SURVIVAL")){
			return match($gameMode){
	...
	public function coreItemStackToNet(Item $itemStack) : ItemStack{
		...
		if($idMeta === null){
			...
			[$id, $meta] = $idMeta;
			$blockRuntimeId = $idMeta[2] ?? ItemTranslator::NO_BLOCK_RUNTIME_ID;
		}
