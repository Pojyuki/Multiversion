...
class MultiLayeredChunkSerializer implements MVChunkSerializer{
	...
	public function serializeFullChunk(Chunk $chunk, int $dimensionId, MVBlockTranslator $blockTranslator, PacketSerializerFactory $factory, ?string $tiles = null) : string{
		$stream = $factory->newEncoder();
		$subChunkCount = $this->getSubChunkCount($chunk, $dimensionId);
		...
		for($x = 0; $x < 16; ++$x){
			for($z = 0; $z < 16; ++$z){
				...
				$biome[($z << 4) | $x] = chr($chunk->getBiomeId($x, $y, $z));
			}
		}
		$stream->put($biome);
	...
	public function serializeTiles(Chunk $chunk) : string{
		...
		foreach($chunk->getTiles() as $tile){
			if($tile instanceof Spawnable){
				$stream .= $tile->getSerializedSpawnCompound()->getEncodedNbt();
			}
		}
