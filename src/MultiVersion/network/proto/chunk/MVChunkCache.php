...
class MVChunkCache implements ChunkListener{
	...
	/** @var array<int, CompressBatchPromise|string> */
	 ...
	 * @param int $chunkZ
	 *
	 * @return CompressBatchPromise|string a promise of resolution or an already compressed chunk packet.
	 */
	...
	private function prepareChunkAsync(int $chunkX, int $chunkZ, int $chunkHash) : CompressBatchPromise{
		...
		try{
			$promise = $this->caches[$chunkHash] = new CompressBatchPromise();
			...
			$promise->onResolve(function(CompressBatchPromise $promise) use ($chunkHash) : void{
				//The promise may have been discarded or replaced if the chunk changed while it was prepared.
	...
	private function restartPendingRequest(int $chunkX, int $chunkZ) : void{
		...
		$this->prepareChunkAsync($chunkX, $chunkZ, $chunkHash)->onResolve(...$existing->getResolveCallbacks());
	}
