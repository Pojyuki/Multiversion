...
class v419PacketSerializer extends MVPacketSerializer{
	...
	public static function reader(ByteBufferReader $reader, ?int $protocolId = null) : static{
		return parent::reader($reader, v419PacketTranslator::PROTOCOL_VERSION);
	}

	public static function writer(ByteBufferWriter $writer, ?int $protocolId = null) : static{
		return parent::writer($writer, v419PacketTranslator::PROTOCOL_VERSION);
	}
	...
	public function getGameRules() : array{
		...
		for($i = 0; $i < $count; ++$i){
			...
			$rules[$name] = match($type){
				GameRuleType::BOOL => new BoolGameRule($this->getBool(), true),
				GameRuleType::INT => new IntGameRule($this->getUnsignedVarInt(), true),
				GameRuleType::FLOAT => new FloatGameRule($this->getLFloat(), true),
	...
	public function putGameRules(array $rules) : void{
		...
		foreach($rules as $name => $rule){
			...
			match($rule->getTypeId()){
				GameRuleType::BOOL => $this->putBool(($rule instanceof BoolGameRule) ? $rule->getValue() : false),
				GameRuleType::INT => $this->putUnsignedVarInt(($rule instanceof IntGameRule) ? $rule->getValue() : 0),
				GameRuleType::FLOAT => $this->putLFloat(($rule instanceof FloatGameRule) ? $rule->getValue() : 0.0),
				default => throw new InvalidArgumentException("Unknown gamerule type " . $rule->getTypeId())
			};
		}
	}
}
