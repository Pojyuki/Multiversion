...
use MultiVersion\network\proto\v419\packets\types\inventory\stackrequest\v419CreativeCreateStackRequestAction;
use MultiVersion\network\proto\v419\packets\types\inventory\stackrequest\v419DeprecatedCraftingResultsStackRequestAction;
...
class v419ItemStackRequestExectutor{
	...
	protected function processItemStackRequestAction(ItemStackRequestAction $action) : void{
		if(
		...
		}elseif($action instanceof DestroyStackRequestAction || $action instanceof v419DestroyStackRequestAction){
			$destroyed = $this->removeItemFromSlot($action->getSource(), $action->getCount());
			$this->builder->addAction(new DestroyItemAction($destroyed));

		}elseif($action instanceof v419CreativeCreateStackRequestAction){
			...
			// Use protocol-specific roundtrip result so server creates what the v419 client actually saw.
			try{
		...
		}elseif($action instanceof v419CraftRecipeStackRequestAction || $action instanceof CraftRecipeStackRequestAction){
		...
		}elseif($action instanceof CraftingConsumeInputStackRequestAction || $action instanceof v419CraftingConsumeInputStackRequestAction){
		...
		}elseif($action instanceof DeprecatedCraftingResultsStackRequestAction || $action instanceof v419DeprecatedCraftingResultsStackRequestAction){
			//no obvious use
		}else{
