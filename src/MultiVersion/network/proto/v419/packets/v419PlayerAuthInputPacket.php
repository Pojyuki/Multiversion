...
class v419PlayerAuthInputPacket extends PlayerAuthInputPacket{
	private const LEGACY_PLAY_MODE_VR = 4;

	public function hasFlag(int $flag) : bool{
		return $this->getInputFlags()->get($flag);
	}
	...
	protected function decodePayload(\pmmp\encoding\ByteBufferReader $in, ?int $protocolId = null) : void{
		...
		$legacyInputFlags = $in->getUnsignedVarLong();
		ReflectionUtils::setProperty(PlayerAuthInputPacket::class, $this, "inputFlags", new BitSet(PlayerAuthInputFlags::NUMBER_OF_FLAGS, [(int) $legacyInputFlags]));
		...
		if(ReflectionUtils::getProperty(PlayerAuthInputPacket::class, $this, "playMode") === self::LEGACY_PLAY_MODE_VR){
		...
		if($this->getInputFlags()->get(PlayerAuthInputFlags::PERFORM_BLOCK_ACTIONS)){
			...
			for($i = 0; $i < $max; ++$i){
				...
				$blockActions[] = match (true) {
					PlayerBlockActionWithBlockInfo::isValidActionType($actionType) => PlayerBlockActionWithBlockInfo::read($in->getReader(), $actionType),
