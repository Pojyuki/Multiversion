...
class v486ClientboundMapItemDataPacket extends ClientboundMapItemDataPacket{
	...
	protected function decodePayload(\pmmp\encoding\ByteBufferReader $in, ?int $protocolId = null) : void{
		...
		if(($this->type & self::BITFLAG_DECORATION_UPDATE) !== 0){
			...
			for($i = 0, $count = $in->getUnsignedVarInt(); $i < $count; ++$i){
				...
				$yOffset = $in->getByte();
				$label = $in->getString();
				$color = Color::fromRGBA(self::flipIntEndianness($in->getUnsignedVarInt()));
	...
	protected function encodePayload(\pmmp\encoding\ByteBufferWriter $out, ?int $protocolId = null) : void{
		...
		if(($type & self::BITFLAG_DECORATION_UPDATE) !== 0){
			...
			foreach($this->decorations as $decoration){
				...
				$out->putUnsignedVarInt(self::flipIntEndianness($decoration->getColor()->toRGBA()));
			}
		}
	...
	private static function flipIntEndianness(int $value) : int{
		$value &= 0xffffffff;
