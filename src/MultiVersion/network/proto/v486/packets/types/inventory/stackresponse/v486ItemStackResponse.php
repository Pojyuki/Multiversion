...
final class v486ItemStackResponse{
	...
	public static function read(PacketSerializer $in) : self{
		...
		for($i = 0, $len = $in->getUnsignedVarInt(); $i < $len; ++$i){
			...
			$containerInfos[] = new v486ItemStackResponseContainerInfo($containerInfo->getContainerId(), $containerInfo->getSlots());
		}
	...
	public function write(PacketSerializer $out) : void{
		...
		foreach($this->containerInfos as $containerInfo){
			(new v486ItemStackResponseContainerInfo($containerInfo->getContainerName()->getContainerId(), $containerInfo->getSlots()))->write($out);
		}
	}
}
