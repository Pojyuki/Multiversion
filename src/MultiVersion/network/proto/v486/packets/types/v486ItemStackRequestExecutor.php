...
use MultiVersion\network\proto\v486\packets\types\inventory\stackrequest\v486DropStackRequestAction;
use MultiVersion\network\proto\v486\packets\types\inventory\stackrequest\v486CraftingConsumeInputStackRequestAction;
use MultiVersion\network\proto\v486\packets\types\inventory\stackrequest\v486CraftRecipeStackRequestAction;
use MultiVersion\network\proto\v486\packets\types\inventory\stackrequest\v486CreativeCreateStackRequestAction;
use MultiVersion\network\proto\v486\packets\types\inventory\stackrequest\v486DestroyStackRequestAction;
use MultiVersion\network\proto\v486\packets\types\inventory\stackrequest\v486ItemStackRequestSlotInfo;
use MultiVersion\network\proto\v486\packets\types\inventory\stackrequest\v486PlaceStackRequestAction;
use MultiVersion\network\proto\v486\packets\types\inventory\stackrequest\v486SwapStackRequestAction;
use MultiVersion\network\proto\v486\packets\types\inventory\stackrequest\v486TakeStackRequestAction;
...
class v486ItemStackRequestExecutor{
	private TransactionBuilder $builder;

	/** @var v486ItemStackRequestSlotInfo[] */
	 ...
	 * @throws ItemStackRequestProcessException
	 */
	protected function getBuilderInventoryAndSlot(v486ItemStackRequestSlotInfo|ItemStackRequestSlotInfo $info) : array{
		$info = $this->normalizeSlotInfo($info);
		try{
			...
			throw new ItemStackRequestProcessException($exception->getMessage(), 0, $exception);
		}
	...
	private function normalizeSlotInfo(v486ItemStackRequestSlotInfo|ItemStackRequestSlotInfo $slotInfo) : v486ItemStackRequestSlotInfo{
		if($slotInfo instanceof v486ItemStackRequestSlotInfo){
			return $slotInfo;
		}

		return new v486ItemStackRequestSlotInfo(
			$slotInfo->getContainerName()->getContainerId(),
			$slotInfo->getSlotId(),
			$slotInfo->getStackId()
		);
	}
	 ...
	 * @throws ItemStackRequestProcessException
	 */
	protected function transferItems(v486ItemStackRequestSlotInfo $source, v486ItemStackRequestSlotInfo $destination, int $count) : void{
	 ...
	 * @throws ItemStackRequestProcessException
	 */
	protected function removeItemFromSlot(v486ItemStackRequestSlotInfo|ItemStackRequestSlotInfo $slotInfo, int $count) : Item{
		$slotInfo = $this->normalizeSlotInfo($slotInfo);
	 ...
	 * @throws ItemStackRequestProcessException
	 */
	protected function addItemToSlot(v486ItemStackRequestSlotInfo|ItemStackRequestSlotInfo $slotInfo, Item $item, int $count) : void{
		$slotInfo = $this->normalizeSlotInfo($slotInfo);
		$this->requestSlotInfos[] = $slotInfo;
		[$inventory, $slot] = $this->getBuilderInventoryAndSlot($slotInfo);
		if($count < 1){
	...
	protected function processItemStackRequestAction(ItemStackRequestAction $action) : void{
		if(
		...
		}elseif($action instanceof v486SwapStackRequestAction){
		...
		}elseif($action instanceof DestroyStackRequestAction || $action instanceof v486DestroyStackRequestAction){
			$destroyed = $this->removeItemFromSlot($action->getSource(), $action->getCount());
			$this->builder->addAction(new DestroyItemAction($destroyed));

		}elseif($action instanceof v486CreativeCreateStackRequestAction){
			...
			// Use the protocol-specific roundtrip result so the server creates what the v486 client actually saw.
			try{
				$typeConverter = v486TypeConverter::getInstance()->getTypeConverter();
				$translated = $typeConverter->netItemStackToCore($typeConverter->coreItemStackToNet($item));
				if(!$translated->isNull()){
					$translated->setCount($item->getCount());
					$item = $translated;
				}
			}catch(\Throwable){
				// Keep original item on conversion failure to preserve old behaviour.
			}

			$this->setNextCreatedItem($item, true);
		}elseif($action instanceof v486CraftRecipeStackRequestAction || $action instanceof CraftRecipeStackRequestAction){
		...
		}elseif($action instanceof CraftingConsumeInputStackRequestAction || $action instanceof v486CraftingConsumeInputStackRequestAction){
