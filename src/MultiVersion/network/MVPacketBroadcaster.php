...
class MVPacketBroadcaster implements PacketBroadcaster{
	...
	private static function encodePacketTimedCompat(ByteBufferWriter $writer, int $protocolId, ClientboundPacket $packet) : string{
		...
		if(self::$networkEncodePacketTimedExpectsProtocol){
	...
	public function broadcastPackets(array $recipients, array $packets) : void{
		...
		foreach($recipients as $recipient){
			if($recipient->getPacketTranslator() !== $this->packetTranslator){
		...
		foreach($packets as $packet){
			...
			$buffer = self::encodePacketTimedCompat(new ByteBufferWriter(), $this->packetTranslator::PROTOCOL_VERSION, $pk);
		...
		foreach($targetsByCompressor as $compressorId => $compressorTargets){
			...
			if(count($compressorTargets) > 1 && $threshold !== null && $totalLength >= $threshold){
				...
				$promise = MVBatch::prepareBatch($batchBuffer, $this->packetTranslator, $compressor, timings: Timings::$playerNetworkSendCompressBroadcast);
