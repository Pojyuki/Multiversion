...
class MVRakLibInterface extends RakLibInterface{
	...
	/**
	 * Supports both upstream PMMP and forks (e.g. NetherGames) where
	 * StandardPacketBroadcaster requires an extra protocolId constructor arg.
	 */
	private static function createPacketBroadcaster(Server $server) : StandardPacketBroadcaster{
		$ctor = (new \ReflectionClass(StandardPacketBroadcaster::class))->getConstructor();
		$required = $ctor?->getNumberOfRequiredParameters() ?? 0;
		if($required >= 2){
	...
	public function __construct(Server $server, string $ip, int $port, bool $ipV6){
		...
		$packetBroadcaster = self::createPacketBroadcaster($server);
